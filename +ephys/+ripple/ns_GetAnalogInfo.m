...
if strcmp(fileInfo.FileTypeID, 'NEURALCD') || strcmp(fileInfo.FileTypeID, 'NEUCDFLT')
    ...
    entityList = find([hFile.Entity(:).FileType] == [hFile.Entity(EntityID).FileType]); % [JCL-20210804: may have issues if the list sorting ever changes]
    chanIDX = find(entityList == EntityID,1,'first');
    ...
    switch(FileTypeID)
        ...
        case 'NEUCDFLT'
            ...
            fseek(fileInfo.FileID, 2, 0);
            minMaxAnalog = fread(fileInfo.FileID, 2, 'single');
        otherwise
    end
