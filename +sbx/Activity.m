...
classdef Activity < dj.Computed
    ...
    methods
        function m = tuning(tbl,expts, conditions,neurons,pv)
            ...
            arguments
                tbl (1,1) sbx.Activity      % Activity table
                expts                       % Experiment table
                conditions                  % Condition table or a combination of plugin/parameter that defines each condition
                neurons                     % Neurons to include
                pv.graph (1,1) logical = true % Shwo a graph
                pv.polar (1,1) logical = false % The independent variable is in degrees; show polarplot
                pv.average (1,1) logical  = false  % Show the average over neurons (with standard error)
                pv.act {mustBeText} = 'spikes';    % Which activity to plot
                pv.start = seconds(1)              % Left edge of time window
                pv.stop = seconds(3)               % Right edge of time window
            end

            if isa(expts,'ns.Experiment')
                expts = fetch(expts);
            end
