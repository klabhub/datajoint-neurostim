%{
# Fluorescence and spiking activity of a ROI in an experiment, based on a specific preprocessed data set
-> sbx.Preprocessed
-> ns.Experiment
roi : smallint    #  id within this segmentation and plane
plane : smallint #  plane
---
time :      longblob   # Time of each frame (in *milliseconds* on the neurostim clock).
...
classdef CRoi < dj.Imported
    ...
    methods
        ...
        function v = get.keySource(~)
            % Limit to experiments that have sbx files
            v = sbx.Preprocessed*(ns.Experiment & (ns.File & 'extension=''.sbx'''));
        end
    end
    ...
    methods (Access=protected)
        ...
        %% Read the NPY Output 
            ...
            for pl = 1:numel(planes)
                ...
                meanrate = mean(spks(:,keepFrameIx),2,"omitnan")/frameDuration;
                stdrate = std(spks(:,keepFrameIx),0,2,"omitnan")/frameDuration;
                ...
                channelTpl = cat(1, mergestruct(key, ...
                                    struct('roi',num2cell(1:nrROIs)', ...
                                        'plane',pl-1,...
                                        'signal',num2cell(spks',1)',...
                                        'name','spikes',...
                                        'channel',1,...
                                        'info',struct)),...
                                mergestruct(key, ...
                ...
                tic;
                for i=1:CHUNK:nrROIs
                    insert(sbx.CRoi,tpl(i:min(i+CHUNK-1,nrROIs)));
                end

                   for i=1:CHUNK:nrROIs
                    insert(sbx.CRoiChannel,channelTpl(i:min(i+CHUNK-1,nrROIs)));
                end
                fprintf('Done in %s \n',seconds(toc))
            end
        end
