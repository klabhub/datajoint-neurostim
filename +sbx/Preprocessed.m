...
%
% With suite2p preprocessing, the fast_disk option can be set in ns.Parms,
...
classdef Preprocessed < dj.Imported
    ...
    methods
        ...
        function v = get.keySource(~)
            % Restrict to sessions that have sbx file and parameters
            % ettings that target the Preprocessed table.
            v =(ns.Session & (ns.File & 'extension=''.sbx'''))* (ns.Parms & 'target=''Preprocessed''');
        end

        function v =  get.ops(tbl)
            % Retrieve the parameters used by suite2p
            keyCntr = 0;
            for key =fetch(tbl*(ns.Parms & 'target=''Preprocessed'''),'*')
                keyCntr = keyCntr+1;
                sessionPath=unique(folder(ns.Experiment & key));
                resultsFolder = [key.subject '.' key.parms.toolbox '.' key.tag];
    ...
    methods (Access=public)
        function v = getFolder(tbl)
            ...
            parms = fetch(ns.Parms & 'target=''Preprocessed''','parms');
            session =fetch(ns.Session & tbl,'subject');
            v = fullfile(sessionPath, [session.subject '.' parms.parms.toolbox '.' parms.tag]);
            if ~exist(v,'dir')
    ...
    methods (Access=protected)
        ...
        function makeTuples(tbl,key)
            sessionPath=unique(folder(ns.Experiment & key));
            parms = fetch1(ns.Parms & 'target=''Preprocessed''' & key,'parms');
            % Set the output folder to be
            % subject.suite2p.preprocessing
            % in the session folder
            resultsFolder = [key.subject '.' parms.toolbox '.' key.tag];
            ...
            switch (parms.toolbox)
                case 'suite2p'
                    ...
                    %settings
                    fn= fieldnames(parms);
                    fn = setdiff(fn,{'toolbox'});
                    for  f= 1:numel(fn)
                        try
                            default= opts{fn{f}};
                            pyClass =class(default);
                            prepValue =parms.(fn{f});
