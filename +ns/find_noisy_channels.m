...
%             A robust z-score function (e.g., calculate_robust_z defined below or gen.robust_z).
...
% --- Argument Validation using arguments block ---
    ...
    if ~isempty(options.Timepoints) && length(options.Timepoints) ~= nTimepoints_sig
        error('find_noisy_channels:InvalidTimepoints', ...
              ...
              length(options.Timepoints), nTimepoints_sig);
    end
    if ~isempty(options.ChannelLocations) && size(options.ChannelLocations, 1) ~= nChannels_sig
         error('find_noisy_channels:InvalidChannelLocations', ...
               'Number of rows in ''ChannelLocations'' (%d) must match the number of channels in ''signal'' (%d).', ...
               size(options.ChannelLocations, 1), nChannels_sig);
    end
    ...
    runRansacCheck = options.EnableRansac && ~isempty(channelCoords); % Check if possible AND enabled
    ...
    if exist('+gen/robust_z','file')
        robust_z_func = @gen.robust_z; fprintf('INFO: Using gen.robust_z.\n');
    ...
    if runRansacCheck
        ...
        switch options.interpolationMethod
            ...
            case "spherical_spline"
                ...
                interp_func = @ns.interpolate_by_spherical_spline;
                interpParams = options.ransacSplineParams;
...
% Calculates robust z-score: (x - median) / (0.7413 * IQR)
% ~ arg is ignored, present for compatibility if switching from gen.robust_z
