                ...
                switch tpl.parms.method
                    ...
                    case "eta"
                        ...
                        assert(isfield(tpl.parms, 'window'), ...
                            'eta method requires a "window" field (half-window size in ms around each event).');
                        assert(isfield(tpl.parms, 'baseline'), ...
                            'eta method requires a "baseline" field (baseline duration in ms at the start of the window).');
                        assert(isnumeric(tpl.parms.window) && isscalar(tpl.parms.window) && tpl.parms.window > 0, ...
                            '"window" must be a positive scalar (ms).');
                        assert(isnumeric(tpl.parms.baseline) && isscalar(tpl.parms.baseline) && tpl.parms.baseline > 0, ...
