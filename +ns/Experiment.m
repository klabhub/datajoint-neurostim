    ...
    methods (Access = public)
        ...
        function [out,filename] = get(tbl,plg,pv)
            ...
            for exptKey=tbl.fetch('paradigm')'
                ...
                if any(notFound)
                    warnNoTrace('This experiment (%s:%s/%s@%s) did not use the %s plugin(s) ',false,exptKey.paradigm,exptKey.subject,exptKey.session_date,exptKey.starttime, strjoin(plg(notFound),'/'));
                    out{exptCntr} = [];
