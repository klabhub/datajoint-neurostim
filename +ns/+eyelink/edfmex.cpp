...
{	
	...
	mexPrintf("Eyelink Loading:%3i%%",0);
...
//				mexPrintf(" *");
	...
	mxAddField(OutputMexObject,"NUMTRIALS");
	mxArray *nrTrials  = mxCreateDoubleMatrix(1,1,mxREAL);
	*mxGetPr(nrTrials) = Ntrial;
	mxSetField(OutputMexObject,0,"NUMTRIALS",nrTrials);

	mxArray *nrElms  = mxCreateDoubleMatrix(1,1,mxREAL);
	*mxGetPr(nrElms) = Nrec;
	mxAddField(OutputMexObject,"NUMEL");
	mxSetField(OutputMexObject,0,"NUMEL",nrElms);
